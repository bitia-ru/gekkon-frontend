...
export const TimeFromNow = (datetime) => {
    let secondsFromNow = moment().diff(datetime, 'seconds');
    if (secondsFromNow < 0) {
        for (let i in LOOK_UP_FUTURE_DATE_FORMATTER) {
            let s = parseInt(Object.keys(LOOK_UP_FUTURE_DATE_FORMATTER[i])[0], 10);
