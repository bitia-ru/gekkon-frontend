...
class RoutesShowModal extends Component {
    ...
    onLikeChange = () => {
        if (this.state.isLiked) {
            Axios({url: `${ApiUrl}/v1/likes/${this.state.likeId}`, method: 'delete'})
                .then(response => {
                    this.reloadLikes();
                }).catch(error => {
                alert(error);
    ...
    saveMark = () => {
        ...
        Axios({url: `${ApiUrl}/v1/routes/${this.props.route.id}`, method: 'patch', params: params})
            .then(response => {
    ...
    changeAscentResult = () => {
        if (this.state.ascent) {
            ...
            Axios({url: `${ApiUrl}/v1/ascents/${this.state.ascent.id}`, method: 'patch', params: params})
                .then(response => {
                    this.reloadAscents();
                }).catch(error => {
                alert(error);
